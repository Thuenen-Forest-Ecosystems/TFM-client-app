...
abstract class IColumnState {
  ...
  void moveColumn({required TrinaColumn column, required TrinaColumn targetColumn});
  ...
  bool limitMoveColumn({required TrinaColumn column, required TrinaColumn targetColumn});
...
mixin ColumnState implements ITrinaGridState {
  ...
  void removeColumns(List<TrinaColumn> columns) {
    ...
    refColumns.removeWhereFromOriginal((column) => removeKeys.contains(column.key));
  ...
  void showSetColumnsPopup(BuildContext context) {
    ...
    final toRow = _toRowByColumnField(titleField: titleField, columnField: columnField);
  ...
  @override
  bool limitResizeColumn(TrinaColumn column, double offset) {
    // Allow frozen columns to be resized freely (both wider and narrower)
    return false;
  }
  ...
  void _callOnSorted(TrinaColumn column, TrinaColumnSort oldSort) {
    if (sortOnlyEvent) {
      eventManager!.addEvent(TrinaGridChangeColumnSortEvent(column: column, oldSort: oldSort));
    }
  ...
  void _updateAfterHideColumn({required List<TrinaColumn> columns, required bool notify}) {
    refColumns.update();
  ...
  bool _updateResizeColumns({required TrinaColumn column, required double offset}) {
    ...
    final resizeHelper = getColumnsResizeHelper(columns: columns, column: column, offset: offset);
