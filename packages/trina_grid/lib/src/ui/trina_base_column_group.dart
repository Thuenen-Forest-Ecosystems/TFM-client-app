...
class TrinaBaseColumnGroup extends StatelessWidget implements TrinaVisibilityLayoutChild {
  ...
  TrinaBaseColumnGroup({required this.stateManager, required this.columnGroup, required this.depth})
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        _ColumnGroup(stateManager: stateManager, columnGroup: columnGroup, depth: _childrenDepth),
      ],
    );
  }
}
...
class _ExpandedColumn extends StatelessWidget {
  ...
  const _ExpandedColumn({required this.stateManager, required this.column, required this.height});

  @override
  Widget build(BuildContext context) {
    return TrinaBaseColumn(stateManager: stateManager, column: column, columnTitleHeight: height);
  }
}
...
class _ColumnGroupTitle extends StatelessWidget {
  ...
  EdgeInsets get _padding =>
      columnGroup.group.titlePadding ?? stateManager.configuration.style.defaultColumnTitlePadding;
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: GestureDetector(
        onTap: columnGroup.group.onTap != null
            ? () => columnGroup.group.onTap!(columnGroup.group, stateManager)
            : null,
        child: DecoratedBox(
          ...
          child: Padding(
            padding: _padding,
            child: Align(
              alignment: columnGroup.group.titleTextAlign.alignmentValue,
              child: Row(
                ...
                children: [
                  ...
                  if (columnGroup.group.onTap != null)
                    Padding(
                      padding: const EdgeInsets.only(left: 4.0),
                      child: Icon(
                        columnGroup.group.isCollapsed ? Icons.chevron_right : Icons.expand_more,
                        size: 16,
                        color: style.columnTextStyle.color ?? Colors.white70,
                      ),
                    ),
                ],
              ),
...
class _ColumnGroup extends StatelessWidget {
  ...
  const _ColumnGroup({required this.stateManager, required this.columnGroup, required this.depth});

  List<TrinaColumnGroupPair> get _separateLinkedGroup => stateManager.separateLinkedGroup(
...
class ColumnGroupLayout extends MultiChildLayoutDelegate {
  ...
  Size getSize(BoxConstraints constraints) {
    ...
    var totalWidthOfGroup = separateLinkedGroups.fold<double>(
      ...
      (previousValue, element) =>
          previousValue +
          element.columns.fold(0, (previousValue, element) => previousValue + element.width),
    );
  ...
  void performLayout(Size size) {
    ...
    for (TrinaColumnGroupPair pair in items) {
      ...
      var boxConstraints = BoxConstraints.tight(Size(width, totalHeightOfGroup));
...
class ColumnsLayout extends MultiChildLayoutDelegate {
  ...
  ColumnsLayout({required this.stateManager, required this.columns, required this.textDirection})
    : super(relayout: stateManager.resizingChangeNotifier);
  ...
  Size getSize(BoxConstraints constraints) {
    ...
    double width = columns.fold(0, (previousValue, element) => previousValue + element.width);
  ...
  void performLayout(Size size) {
    ...
    for (TrinaColumn col in items) {
      ...
      var boxConstraints = BoxConstraints.tight(Size(width, totalColumnsHeight));
