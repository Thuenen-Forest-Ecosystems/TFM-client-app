...
class TrinaColumnGroup {
  ...
  /// Optional callback when the group title is tapped.
  final void Function(TrinaColumnGroup group, TrinaGridStateManager stateManager)? onTap;

  /// Tracks whether this group is currently collapsed (columns hidden).
...
class TrinaColumnGroupPair {
  ...
  TrinaColumnGroupPair({required this.group, required this.columns})
    : // a unique reproducible key
      _key = ValueKey(
        group.key.toString() +
            columns.fold("", (previousValue, element) => "$previousValue-${element.field}"),
      );
