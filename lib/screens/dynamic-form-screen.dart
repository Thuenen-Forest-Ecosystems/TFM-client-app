...
class _DynamicFormScreenState extends State<DynamicFormScreen> {
  ...
  Future<Map> getPreviousData() async {
    ...
    Map<String, dynamic>? data = await db.get('SELECT * FROM records WHERE id = ?', [
      widget.recordsId,
    ]);
    previousData = Map<String, dynamic>.from(data);
    previousData['previous_properties'] = previousData['previous_properties'] != null
        ? jsonDecode(previousData['previous_properties'])
        : {};
    setState(() {});
    return data;
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done) {
            if (snapshot.hasData && snapshot.data != null && snapshot.data!['schema'] != null) {
              ...
              Map<String, dynamic> plotSchemaData =
                  clusterSchemaData['properties']['plot']['items'];

              // Make sure it's a valid map
              return JsonSchemaFormWrapper(
                schema: plotSchemaData,
                formData: {},
